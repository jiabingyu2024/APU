...
| `axis_job_decoder.sv` | 已完成，待HDL编译 | 4-beat header、payload、TLAST和错误排空 |
| `apu_stream_loader.sv` | 已完成，待HDL编译 | 32/64-bit payload写各RAM |
| `axis_result_streamer.sv` | 已完成，待HDL编译 | 同步RAM读延迟和结果packet输出 |
| `apu_job_ctrl.sv` | 已完成，待HDL编译 | LOAD/RUN/READ/END、timeout和错误恢复 |
| `apu_dma_perf_counters.sv` | 已完成，待HDL编译 | 字节、busy和stall周期计数 |
| `apu_dma_axil_regs.sv` | 已完成，待HDL编译 | 只读状态、W1C中断、计数清零 |
| `apu_dma_core.sv` | 已完成，待HDL编译 | 复用现有计算叶子并替换AHB封装 |
| `apu_dma_top.sv` | 已完成，待HDL编译 | AXIS、AXI-Lite和内部模块连接 |

按用户边界，本阶段未运行Vivado elaboration、综合、实现或bitstream。Python协议测试通过，
不等价于RTL已经通过编译。
...
2. AXIS和APU均为100 MHz单时钟，未实现CDC。
3. 首版一次只执行一个job，不支持多个job排队。
4. 完整网络参数打包和bit-exact对比尚未上板执行。
5. Vivado第一次打开后必须先确认模块引用成功识别三个总线接口。
